...
function smoothDampVec2(
    ...
    const expVal = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x);
    ...
    currentVelocity.multiplyScalar(expVal);

    out.copy(target.clone().add(change.add(temp).multiplyScalar(expVal)));
...
function smoothDampFloat(
    ...
    velRef.v = (velRef.v - omega * temp) * expVal;

    let out = target + (change + temp) * expVal;
...
function medianPush(buf: number[], v: number, maxLen: number): void {
    buf.push(v);
...
export const GridScan: React.FC<GridScanProps> = ({
    ...
    // Live-update uniforms without recreating the renderer
    useEffect(() => {
        ...
        if (bloomRef.current) {
            bloomRef.current.blendMode.opacity.value = Math.max(
                0,
                bloomIntensity,
            );
            (
